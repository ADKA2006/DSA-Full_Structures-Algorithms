...
class HeapSortClass {
    ...
    public:
        HeapSortClass() = default;
        void sort(T *items,int n);
};

template <class T>
void HeapSortClass<T>::fixDown(T *items,int k,int n){
    int ptr = k;
    int ptr1, ptr2;
    while (2*k + 1 < n){  
        ptr1 = (2*k + 1);     
...
template <class T>
void HeapSortClass<T>::do_heapify(T *items,int n){
    Heapify<T> heapify;
    heapify.heapify(items,n);
}

template <class T>
void HeapSortClass<T>::sort(T *items,int n){
    do_heapify(items,n);
    for (int i = n - 1; i > 0; i--) {
        T temp = items[i];
        items[i] = items[0];
